...
local _writecustomasset = writecustomasset and (writecustomasset);
...
do
    ...
    do
        ...
        function quad:update()
            ...
            if not frame._line1 then return end;
        ...
        function quad:__namecall(self, k)
        ...
        function quad:_calculateFill() --thx sou btw
            local props = self._properties
    
            local pointA = props.PointA;
            local pointB = props.PointB;
            local pointC = props.PointC;
            local pointD = props.PointD;
            ...
            local minX = _mathmin(pointA.X, pointB.X, pointC.X, pointD.X)
            local maxX = _mathmax(pointA.X, pointB.X, pointC.X, pointD.X)
            local minY = _mathmin(pointA.Y, pointB.Y, pointC.Y, pointD.Y)
            local maxY = _mathmax(pointA.Y, pointB.Y, pointC.Y, pointD.Y)
            ...
            self._frame._filled.Size = _udim2fromoffset(width, height + 2)
        end
